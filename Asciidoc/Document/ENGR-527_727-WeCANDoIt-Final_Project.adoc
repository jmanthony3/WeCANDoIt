...
To further demonstrate the importance of performing a mesh refinement study, examine xref:fig-fea_mesh_refinement-contact_line[].
Measured at the contacting line between the crushing face of the cam and the top of the can (best visualized in xref:fig-fea-cam[]), the stress normal to the axis of the shaft was observed to vary radially.
...
[#fig-fea_mesh_refinement-contact_line]
...
<%!
    ...
    perc_from_pred = np.abs(tau_max_pred - tau_max_shaft)/np.average([tau_max_pred, tau_max_shaft])*100 # %
%>
...
Therefore, selection of this hexagonal shaft was sufficient for this design and the results from the FEA study differed from the analytical calculation (stem:[8.646~MPa]) by stem:[${engr(perc_from_pred)}\%]
...
[#fig-fea_handle]
.A maximum bending stress of stem:[37~MPa] occurs at the point in the arm of the handle close to the shaft.
image::fea_handle.png[caption=<span class="floatnumber">Figure {secs}-{counter:figs}. </span>, reftext="Fig. {secs}-{figs}"]

===== Cam
[#fig-fea-cam]
.The maximum normal stress in the shaft axial direction of stem:[\sim 7~MPa] (xref:fig-fea_mesh_refinement-contact_line[]) occurs at the rightmost extreme of the contact line as shown.
...
%>
The yield strength of AISI 316 stainless steel is listed in *SolidWorks* as stem:[${yield_strength_AISI316}~MPa]; however, McMaster-Carr lists stem:[${yield_strength_CARR}~MPa].
...
// speaking on the chamber
This gargantuan stress indicates that further work is needed in the design of the chamber housing to properly handle stress incurred from nominal operation.
